...
public: //------------------------------------------------------ property types
    ...
    public:
        ...
        /// access the data stored for vertex \c v1
        typename Property<T>::reference operator[](Vertex v1)
        {
            return Property<T>::operator[](v1.idx());
        }

        /// access the data stored for vertex \c v1
        typename Property<T>::const_reference operator[](Vertex v1) const
        {
            return Property<T>::operator[](v1.idx());
        }
    };
...
bool read_poly(Surface_mesh& mesh, const std::string& filename)
{
    ...
    n_items = (int)fread((char*)vconn.data(), sizeof(Surface_mesh::Vertex_connectivity),   nv, in);
    n_items = (int)fread((char*)hconn.data(), sizeof(Surface_mesh::Halfedge_connectivity), nh, in);
    n_items = (int)fread((char*)fconn.data(), sizeof(Surface_mesh::Face_connectivity),     nf, in);
    n_items = (int)fread((char*)point.data(), sizeof(Point),                               nv, in);
