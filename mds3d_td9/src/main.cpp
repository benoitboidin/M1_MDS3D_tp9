...
static void key_callback(GLFWwindow* window, int key, int /*scancode*/, int action, int mods)
{
    ...
    g_v->keyPressed(key,action,mods);
}

void mouse_button_callback(GLFWwindow* window, int button, int action, int /*mods*/)
{
    g_v->mousePressed(window, button, action);
}

void cursorPos_callback(GLFWwindow* /*window*/, double x, double y)
{
    g_v->mouseMoved((int)(x*g_pixel_ratio),(int)(y*g_pixel_ratio));
}

void reshape_callback(GLFWwindow* window, int width, int height)
{
    g_v->reshape(width,height);
    g_v->updateAndDrawScene();
